...
description: "ANOTHER EDIT: Apparently the hookscript section is what causes me the most headache and is not needed. Since the tap interface is bridged with vmbr0, it should not be configured to get an IP. The correct way to do it is to configure an IP and gateway on the bridge vmbr0 itself like so:
...
EDIT: Apparently it's possible to get rid of the fw* interf"
date: "2024-06-02T21:40:00.000+05:30"
...
EDIT: Apparently it's possible to [get rid of the fw\* interfaces](https://kayg.org/blog/getting-rid-of-fwbr-interfaces-on-proxmox/) when the proxmox firewall is not used, so I am updating the post with the fw\* interfaces removed.
...
![Insides of the Skullsaints Onyx](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729028830.jpg)
...
![./virtual-router-proxmox-skullsaints-onyx/virtual-router-goal.png](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729028861.png)
...
![./virtual-router-proxmox-skullsaints-onyx/virtual-router-poc.png](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729028888.png)
...
Installing OpenWRT was made very easy by this person's gist: [https://gist.github.com/ryuheechul/e829b7846a965638deeb122bdd0b403c](https://gist.github.com/ryuheechul/e829b7846a965638deeb122bdd0b403c?ref=kayg.org)
...
![./virtual-router-proxmox-skullsaints-onyx/virtual-router-nic-passthrough.png](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729028916.png)
...
![./virtual-router-proxmox-skullsaints-onyx/virtual-router-openwrt-configuration.png](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729028944.png)

Once that's done, set the interface `br-lan` to be a DHCP client, so it asks for an IP assignment from our external gateway.

![./virtual-router-proxmox-skullsaints-onyx/virtual-router-openwrt-br-lan-dhcp.png](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729028972.png)

Here's an overview of what everything looks like so far:

![./virtual-router-proxmox-skullsaints-onyx/virtual-router-openwrt-interfaces-overview.png](/images/ghost/virtual-router-in-proxmox-with-the-skullsaints-onyx-1768729029001.png)
...
The tap and fwbr interfaces already has an IP address. How can that be? We never configured it to be a dhcp client. That's because two more steps are necessary for Proxmox to get internet and I have already performed them.
...
1.  Run `dhclient` after VM starts and gets an IP address.This is solved by simply running `/usr/sbin/dhclient`.
2.  To get the same IP everytime, one of the interfaces need to have a fixed MAC Address. To set a fixed MAC, we can use `ip link set <dev> address <mac-address>`.And apparently, it needs to be the `fwpr100p` interface. I haven't figured out why yet.
...
1.  Run `systemctl restart networking` once Proxmox's interfaces get an IP address.This is solved by making sure the command runs after `dhclient`.
2.  Make sure the above happens only after OpenWRT's network is ready. If not, wait.This is solved my simply checking whether the VM has had an ip assignment or not: `/usr/sbin/qm guest cmd 100 network-get-interfaces | /usr/bin/grep '101'` (10.0.0.101 is the static lease for my VM by my external gateway).
