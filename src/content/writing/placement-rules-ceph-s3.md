...
Placement rules in [ceph](<./ceph.md>) are a method to assign a specific [[data pool]], [[index pool]] and [[extra pools]] to a particular [[bucket]]. Placement rules are assigned to buckets at the time of creation and cannot be changed after creation. The default placement rule is called, `default-placement`. It assigns a [storage class](<./storage-classes-ceph-s3.md>) called `STANDARD` which links the default [[rados gateway]] data, index and non-ec pools. 
